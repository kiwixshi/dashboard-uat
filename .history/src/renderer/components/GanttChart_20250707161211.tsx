...
export default function GanttChart({breakData: userBreakData, username: username, passData: passUserTimeUsage}:{breakData: breakDataInterface[], username: string, passData:any}) {
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `activity chart of user ${username}`,
      },
    },
  };
  ...
  useEffect(()=>{
    const data1 = userBreakData.map((element)=>[Number(element.intime.substring(11, 13))*60, Number(element.intime.substring(11, 13))*60+Math.round(element.difference/60000)]).sort((a, b)=>{if(a[0]==b[0]){return 0}return (a[0]>b[0])?1:-1});
    getActive(data1);
    let data2 = [];
