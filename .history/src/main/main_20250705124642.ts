...
let dbPath = ["C:\\Users\\shiva\\source\\repos\\UserActivityTrackerService\\x64\\Debug\\session_logs.db"];
...
ipcMain.handle('get-user-ranks', async()=>{
  if(dbPath){
    return selectUserRanking(dbPath[0]);
  }
});

ipcMain.handle('get-avg-time-tdy', async()=>{
  if(dbPath){
    return selectAvgUserTime(dbPath[0]);
  }
...
ipcMain.handle('get-time-in-days', async(event: Electron.IpcMainInvokeEvent, time: number)=>{
  if(dbPath){
    return selectUserTimePerDays(dbPath[0], time);
  }
...
ipcMain.handle('get-active-tdy', async()=>{
  if(!dbPath) return;
  return selectTotalTimeToday(dbPath[0]);
})

ipcMain.handle('get-user-deets-day', async(event: Electron.IpcMainInvokeEvent, username: string, date: string)=>{
  if(!dbPath) return;
  return selectUserTimeBreakdown(dbPath[0], username, date);
})

ipcMain.handle('get-users', async()=>{
  if(!dbPath) return;
  return selectUsers(dbPath[0]);
})
