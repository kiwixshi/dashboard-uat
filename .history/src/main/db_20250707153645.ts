...
export function selectUserTimeBreakdown(dbPath: string, username: string, date: string): Promise<any[]>{
  ...
  return new Promise(
    (resolve, reject) => {
    db.all(
      `select * from sessions where username = '${username}' and datetime(intime) BETWEEN datetime('2025-07-06', 'start of day', '+30 minutes', 'localtime') and datetime('2025-07-06', '+1 day', 'start of day', '+29 minutes', 'localtime') order by intime asc;`,
        (err, rows)=> {
          if (err) reject(err)
          else resolve(rows)
        }
