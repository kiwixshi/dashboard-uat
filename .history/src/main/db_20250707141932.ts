...
export function selectUserTimePerDays(dbPath: string, days: number): Promise<any[]>{
  ...
  return new Promise(
    (resolve, reject) => {
      db.all(`select date(intime) as 'date', username, sum(difference) as net_time from sessions where outtime is not NULL AND datetime(intime) BETWEEN datetime('now', '-${days} days', 'start of day', '+30 minutes', 'localtime') and datetime('now', '+1 day', 'start of day', '+29 minutes', 'localtime') GROUP by date(intime), username order by date(intime);`,
      (err, rows)=>{
        if (err) reject(err);
        else resolve(rows)
      })
